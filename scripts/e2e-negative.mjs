...
async function main() {
  ...
  console.log("[negativo:PULSE#2] status=", pulse2.status, pulse2.text);
  try {
  const p2 = JSON.parse(pulse2.text || "{}");
  if ((p2.ciclos_criados ?? 0) > 0) {
    console.log("[warn] PULSE duplicado retornou ciclos_criados>0; validar por pagamento_id para confirmar que nao criou ciclo extra.");
  }
} catch (e) {
  // ignore parse errors
}
 if (pulse2.status < 200 || pulse2.status >= 300) fail("negativo PULSE duplicado falhou", pulse2.text);
  // Não deve criar ciclo extra; o ciclo original deve permanecer FINALIZADO
  await expectCycleStatus(fin.cycle_id, "FINALIZADO");


  console.log("\n✅ E2E NEGATIVE OK", { pagamento_id: fin.pagamento_id, cycle_id: fin.cycle_id, command_id: cmd_id });
}
