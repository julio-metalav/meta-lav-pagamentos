...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const vigenteAPartirRaw = body?.vigente_a_partir;
    ...
    const vigenteAPartir = vigenteAPartirRaw != null ? new Date(vigenteAPartirRaw) : null;
    if (!vigenteAPartir || !Number.isFinite(vigenteAPartir.getTime())) return jsonErrorCompat("vigente_a_partir é obrigatório e deve ser um timestamp válido.", 400, { code: "invalid_vigente_a_partir" });
    const now = new Date();
    if (vigenteAPartir.getTime() <= now.getTime()) return jsonErrorCompat("vigente_a_partir deve ser maior que agora.", 400, { code: "vigente_a_partir_must_be_future" });
    ...
    const { data: existingRows, error: existErr } = await sb
      ...
      .limit(1);
    if (existErr) return jsonErrorCompat("Erro ao verificar preço futuro.", 500, { code: "db_error", extra: { details: existErr.message } });
    if (existingRows?.length) return jsonErrorCompat("Já existe preço futuro agendado para esta máquina e canal.", 409, { code: "future_price_exists" });
    ...
    const { data: inserted, error: insertErr } = await sb
      ...
      .insert({
        ...
        vigente_a_partir: vigenteAPartir.toISOString(),
        created_by: createdBy,
      })
      .select("id,condominio_maquina_id,canal,valor_centavos,vigente_a_partir")
      .single();

    if (insertErr) return jsonErrorCompat("Erro ao agendar preço.", 500, { code: "db_error", extra: { details: insertErr.message } });

    return NextResponse.json({ ok: true, item: inserted });
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : String(e);
    return jsonErrorCompat("Erro inesperado ao agendar preço.", 500, {
      code: "internal_error",
      extra: { details: msg },
    });
  }
